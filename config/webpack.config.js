...
const {fallback} = require("./webpack.vendor.config").resolve;
...
module.exports = function(webpackEnv) {
	...
	if (isEnvProduction) plugins.push([require.resolve("babel-plugin-transform-remove-console")]);

	if (isEnvDevelopment && shouldUseReactRefresh) plugins.push([require.resolve("react-refresh/babel")]);

	const vendorManifest = path.join(__dirname, "..", "build", "vendor-manifest.json");
	if (isEnvDevelopment && !fs.existsSync(vendorManifest)) {
	...
	return {
		...
		experiments: {
			...
			lazyCompilation: {
				entries: false,
				imports: isEnvDevelopment && process.env.DISABLED_LAZY !== "true",
			},
		},
		cache: {
			type: "memory",
		...
		plugins: [
			...
			new HtmlWebpackPlugin(
				Object.assign(
					isEnvDevelopment ? {vendor: "/vendor.bundle.js"} : {},
					{
						inject: true,
