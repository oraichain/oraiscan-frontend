...
export default class WalletStation {
	...
	signBroadcast = async props => {
		...
		try {
			switch (type) {
				...
				case typeSign.MULTISENDCW20:
					return await client.executeMultiple(fromAddress, msg, gas);
	...
	withdrawDelegatorReward = async msgs => {
		if (!Array.isArray(msgs))
			return this.signBroadcast({ delegator_address: msgs?.delegator_address, validator_address: msgs?.validator_address, type: typeSign.WITHDRAWREWARDS });
		let messages = [];
		for (let msg of msgs) {
			messages.push({
