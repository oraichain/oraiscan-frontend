...
const {COSMOS_SDK, DEX, TOKENS, MISC} = txTypes;
export default function(type) {
	if (_.find([TOKENS.ISSUE, TOKENS.MINT, TOKENS.BURN, DEX.LIST], v => type === v)) return SVG[0];
	else if (_.find([COSMOS_SDK.SEND, DEX.ORDER_NEW, DEX.ORDER_CANCEL], v => type === v)) return SVG[1];
	else if (_.find([COSMOS_SDK.MSG_SUBMIT_PROPOSAL, COSMOS_SDK.MSG_DEPOSIT, COSMOS_SDK.MSG_VOTE], v => type === v)) return SVG[2];
