...
const SmartContractTableSkeleton = memo(({ rows }) => {
	...
	const getDataRows = rows => {
		...
		for (let i = 1; i <= rows; i++) {
			...
			const codeIdDataCell = (
				<div className={cx("align-left")}>
					<Skeleton variant='text' className={cx("skeleton")} width={65} height={21} />
				</div>
			);
			...
			dataRows.push([codeIdDataCell, wasmDataCell, sourceDataCell]);
		}
		return dataRows;
	};
	...
	return <TableCustom theme={tableThemes.LIGHT} headerCellStyles={headerRow.headerCellStyles} headerCells={headerRow.headerCells} dataRows={dataRows} />;
});
