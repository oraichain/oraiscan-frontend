...
const RandomnessViewOld = ({data, errorMessage}) => {
	...
	const handelGetTx = async () => {
		...
		const apiGetTx = `${consts.LCD_API_BASE}${consts.LCD_API.TXS}?events=wasm.round%3D%27${data?.latest?.round}%27&events=wasm.contract_address%3D%27${oldRandomnessContractAddress}%27`;
...
RandomnessViewOld.propTypes = {
	data: PropTypes.any,
};

RandomnessViewOld.defaultProps = {};

export default RandomnessViewOld;
