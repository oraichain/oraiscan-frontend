...
export const ThinTableRow = ({asset = {}}) => {
	...
	return (
		<div key={asset.id} className={cx("ValidatorTxList-thinTableRow")}>
				...
				<li>
					{!_.isNil(formattedValue) ? (
						<span className={cx("flexIt")}>
							...
							{!_.isNil(asset?.txValidator) ? (
								<span className={cx({BNB: asset?.txValidator?.split("-")[0] === "BNB"})}>{asset?.txValidator?.split("-")[0]}</span>
							) : (
								undefined
							)}
						</span>
...
export default function TxTableRows({asset = {}}) {
	...
	return (
		<TableRow className={cx("TxTableRows-wrapper")} hover={true} key={asset.txHash}>
			...
			<TableCell className={cx("tableCell", "text", asset.txValidator === "BNB" ? "BNB" : undefined)} component='th' scope='row'>
				{asset.txValidator ? asset.txValidator : <Skeleton width={"80px"} widthRandomness={0} />}
			</TableCell>
