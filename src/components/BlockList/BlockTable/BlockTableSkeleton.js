...
import {getHeaderRow} from "src/components/BlockList/BlockTable/BlockTable";
...
const BlockTableSkeleton = memo(({rows = 10}) => {
	...
	const getDataRows = rows => {
		...
		for (let i = 1; i <= rows; i++) {
			...
			dataRows.push([heightDataCell, parentHashDataCell, nodeDataCell, txsDataCell, timeDataCell]);
		}
		return dataRows;
	};
